test_that("score_cram_index returns expected output for NC-TEN", {
  ...
  cram <- read.csv(
    system.file("extdata", "EMPA_CRAM_2014-2024.csv", package = "EMPAFunctionAnalysis")
  )
...
test_that("score_cram_index output has correct columns", {
  ...
  expect_named(
    result,
    c("estuaryname", "siteid", "year", "function_name", "indicator_name", "metric_name", "metric_score")
  )
})
...
test_that("score_cram_index produces one row per function_name", {
  ...
  result_both <- score_cram_index(vegetativecover_data = input, cram = cram, function_name = c("Plant", "SLR"))
  result_plant <- score_cram_index(vegetativecover_data = input, cram = cram, function_name = "Plant")
